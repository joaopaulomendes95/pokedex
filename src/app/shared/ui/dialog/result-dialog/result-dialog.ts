...
import { ActionResult } from '@shared/ui/dialog/dialog.interface';
...
export class ResultDialog {
  ...
  #dialogRef = inject(MatDialogRef<ResultDialog, boolean>);

  readonly faIcon = this.dialogData.success ? 'circle-check' : 'circle-xmark';
  readonly title =
    this.dialogData.title ?? (this.dialogData.success ? 'Success' : 'Something went wrong');
