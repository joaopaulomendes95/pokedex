...
```typescript
{avoid, header: 'reads value() directly without status propagation'}
const companyResource = resource({
  params: () => {
...
ng test --debug
``` 2. The test runner will start in debug mode and wait for a debugger to attach. 3. You can now attach your preferred debugger. For example, you can use the built-in Node.js debugger in VS Code or the Chrome DevTools for Node.js.
